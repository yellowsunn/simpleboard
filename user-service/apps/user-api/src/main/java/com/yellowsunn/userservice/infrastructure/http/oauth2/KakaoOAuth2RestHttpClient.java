...
import com.yellowsunn.userservice.infrastructure.http.response.KakaoOAuth2UserInfoResponse;
import com.yellowsunn.userservice.infrastructure.http.response.KakaoOAuth2UserInfoResponse.KakaoUserInfo;
import com.yellowsunn.userservice.infrastructure.http.response.KakaoOAuth2UserInfoResponse.KakaoUserInfo.KakaoUserProfile;
...
public class KakaoOAuth2RestHttpClient implements OAuth2UserInfoHttpClient {
    ...
    public OAuth2UserInfo findUserInfo(String code, OAuth2Request oAuth2Request) {
        ...
        var uri = UriComponentsBuilder.fromUriString(KAKAO_API_BASE_URI)
                ...
                .toUri();
        try {
            var responseEntity = restTemplate.exchange(uri, HttpMethod.GET, request,
                    KakaoOAuth2UserInfoResponse.class);
    ...
    private String getTokenByCode(String code, OAuth2Request oAuth2Request) {
        ...
        var uri = UriComponentsBuilder.fromUriString(KAKAO_AUTH_BASE_URI)
                ...
                .toUri();
        try {
            var kakaoOAuth2TokenResponseDto = restTemplate.postForObject(uri, params,
                    KakaoOAuth2TokenResponse.class);
