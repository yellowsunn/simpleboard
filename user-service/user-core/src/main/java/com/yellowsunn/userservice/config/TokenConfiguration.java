...
public class TokenConfiguration {
    @Bean
    public AccessTokenHandler accessTokenHandler(@Value("${token.access.secret}") String secret,
                                                 @Value("${token.access.expiration}") Duration expiration) {
        return new AccessTokenHandler(secret, expiration);
    }

    @Bean
    public RefreshTokenHandler refreshTokenHandler(@Value("${token.refresh.secret}") String secret,
